...
namespace LinqBookStore
{
    public class Program
    {
        ...
        {
            return books.Where(b => b.Price >= price).OrderBy(b => b.Author.LastName);
        }

        static public IEnumerable<string> GetGenres(List<Book> books)
        {
            return books.Select(s => s.Genre).Distinct().ToList();
        }
        static public IEnumerable<Book> GetBooksByGenre(List<Book> books, string genre)
        {
            return books.Where(b => b.Genre == genre).OrderBy(b => b.Author.LastName).ThenBy(b => b.Title);
        }
        static public IEnumerable<Book> GetBooksByPublishDate(List<Book> books, DateTime date)
        {
            return books.Where(b => b.PublishDate >= date).OrderByDescending(b => b.PublishDate);
        }

        static public IEnumerable<Book> GetBooksByRating(List<Book> books, int rating)
        {
            return books.Where(b => b.Rating == rating).OrderBy(b => b.Title);
        }
        static public void FindRecommendedBooksByGenre(List<Book> books, string genre)
        {
            var ratedBooks = books.Where(b => b.Genre == genre)
                .OrderBy(b => b.Title)
                .Select(s => new 
                {
                    s.Title,
                    Recommended = s.Rating >= 8 ? "Yes" : "No"
                });

            Console.WriteLine($"{"Title",-40} {"Recommended",-15}");
            foreach (var book in ratedBooks)
            {
                Console.WriteLine($"{book.Title,-40} {book.Recommended}");
            }
        }
        static public void FindApproximateSizeOfBooks(List<Book> books)
        {
            var booksBySize = books
                ...
                .Select(s => new
                {
                    ...
                    BookSize = s.NumberOfPages < 200 ? "Small" : s.NumberOfPages >= 200 && s.NumberOfPages <= 400 ? "Medium" : "Long"
                });

            Console.WriteLine($"{"Title",-40} {"Author",-20} Size");
            foreach (var book in booksBySize)
            {
                Console.WriteLine($"{book.Title,-40} {book.Author, -20} {book.BookSize}");
            }
        }
        static public void FindCheapestbooks(List<Book> books)
        {
            var cheapestBooks = books
                ...
                .Select(b => new
                {
                    ...
                    Cheapness = b.Price < 5.00 ? "Dirt Cheap" : "Pretty Cheap"
                });

            Console.WriteLine($"{"Title",-40} {"Author",-20} Cheapness");
            foreach (var book in cheapestBooks)
            {
                Console.WriteLine($"{book.Title,-40} {book.Author,-20} {book.Cheapness}");
            }
        }

        static public int GetNumberOfBooksByGenre(List<Book> books, string genre)
        {
            return books.Count(b => b.Genre == genre);
        }

        static public double GetTotalValueOfBooks(List<Book> books)
        {
            
            return Math.Round(books.Sum(b => b.Price), 2);
        }
        static public IEnumerable<Book> GetCheapestBooks(List<Book> books)
        {
            double lowestPrice = books.Min(b => b.Price);
            return books.Where(b => b.Price == lowestPrice);
        }
        static public IEnumerable<Book> GetMostExpensiveBooks(List<Book> books)
        {
            double highestPrice = books.Max(b => b.Price);
            return books.Where(b => b.Price == highestPrice)    ;
        }
        #endregion
